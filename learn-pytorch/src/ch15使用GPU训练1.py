...
def main():
    ...
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset, batch_size=64, drop_last=True, num_workers=0
    )
    test_dataloader = torch.utils.data.DataLoader(
        test_dataset, batch_size=64, drop_last=True, num_workers=0
    )
    ...
    if os.path.exists("../pretrained_model/ch14_model.pth"):
        ...
        for i in range(eporch):
            ...
            print(
                f"本轮训练损失函数值总和为{loss_result_train},正确率为{1.0 * acurracy / train_size}"
            )
            ...
            print(
                f"本轮测试损失函数值总和为{loss_result_test},正确率为{1.0 * acurracy / test_size}"
            )
            mynet.train()
    ...
    writer.close()
    end_time = time.time()
    print(f"共消耗时间{end_time - start_time}")
